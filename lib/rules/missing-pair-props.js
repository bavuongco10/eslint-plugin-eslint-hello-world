  ...
  create: function (context) {
    ...
    return {
      ...
      "VariableDeclaration": function (node) {
        if (node.declarations[0].id.name !== 'mapStateToProps')  return

        if(!node.declarations[0] || !node.declarations[0].init || !node.declarations[0].init.body || !node.declarations[0].init.body.properties) return
